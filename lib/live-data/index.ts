...
export async function getLiveData(force = false): Promise<LiveDataResponse> {
  ...
  const sources: SourceStatus[] = [
    ...
    { name: "Gemini", status: analysis.status, updatedAt: analysis.timestamp, freshness: freshness(analysis.status, analysis.timestamp) },
  ];
  ...
  return { timestamp: new Date().toISOString(), status, sources, market, geopoliticalRisk: risks, corridorRisk, events: gdelt.value ?? [], gemini: { status: analysis.status, generatedAt: analysis.timestamp, analysis: analysis.value }, dataQuality: { liveSources, staleSources, unavailableSources } };
}
